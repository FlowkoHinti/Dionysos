...
def RegisterShellCommand(shellCommand, exeCommand, shellUserCommand=None):
    ...
    base = "%s\\Shell" % RegistryIDPyFile
    if shellUserCommand:
        win32api.RegSetValue(win32con.HKEY_CLASSES_ROOT, base + "\\%s" % (shellCommand), win32con.REG_SZ,
                             shellUserCommand)

    win32api.RegSetValue(win32con.HKEY_CLASSES_ROOT, base + "\\%s\\Command" % (shellCommand), win32con.REG_SZ,
                         exeCommand)
