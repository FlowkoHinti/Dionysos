...
class DispatchItem(build.DispatchItem, WritableItem):
    ...
    def WriteCallbackClassBody(self, generator):
        ...
        for name, entry in list(self.propMapGet.items()) + list(self.propMapPut.items()) + list(self.mapFuncs.items()):
            ...
            print('#\tdef ' + methName + '(self' + build.BuildCallList(fdesc, entry.names, "defaultNamedOptArg",
                                                                       "defaultNamedNotOptArg", "defaultUnnamedArg",
                                                                       "pythoncom.Missing", is_comment=True) + '):',
                  file=stream)
    ...
    def WriteClassBody(self, generator):
        ...
        names.sort()
        specialItems = {"count": None, "item": None, "value": None,
                        "_newenum": None}  # If found, will end up with (entry, invoke_tupe)
        itemCount = None
        for name in names:
            ...
            if generator.bBuildHidden or not entry.hidden:
                ...
                if entry.wasProperty:
                    print(
                        '\t# The method %s is actually a property, but must be used as a method to correctly pass the arguments' % name,
                        file=stream)
        ...
        for key in names:
            ...
            if generator.bBuildHidden or not entry.hidden:
                ...
                argDesc = ()
                mapEntry = MakeMapLineEntry(details[0], pythoncom.DISPATCH_PROPERTYGET, resultDesc, argDesc, key,
        ...
        print('\t\ttry:', file=stream)
        print('\t\t\tob = self._oleobj_.InvokeTypes(%d,LCID,%d,(13, 10),())' % (pythoncom.DISPID_NEWENUM, invkind),
              file=stream)
        ...
        if specialItems["item"]:
            ...
            print('\t#This class has Item property/method which allows indexed access with the object[key] syntax.',
                  file=stream)
