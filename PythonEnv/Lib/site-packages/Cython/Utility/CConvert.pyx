...
@cname("{{funcname}}")
cdef {
{struct_type}} {{funcname}}(obj) except *:
cdef {
{struct_type}} result
...
@cname("{{cname}}")
cdef object
{{cname}}({{return_type.ctype}}(*f)({{', '.join(arg.type_cname for arg in args)}})
{{except_clause}}):
...
{{'' if return_type.type.is_void else 'return '}}
f({{', '.join(arg.name for arg in args)}})
return wrap
...
cdef inline tuple
{{to_tuple_cname}}({{base_type}} * v, Py_ssize_t
length):
cdef size_t i
cdef object value
