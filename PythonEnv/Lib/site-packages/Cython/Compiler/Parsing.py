...
_parse_escape_sequences_raw, _parse_escape_sequences = [re.compile((
                                                                       # escape sequences:
                                                                           ...
                                                                           (br'\\?' if is_raw else (
                                                                               ...
                                                                               br'U[0-9a-fA-F]{8}|'
                                                                               br'[NxuU]|'
                                                                           # detect invalid escape sequences that do not match above
                                                                           )) +
                                                                           br')?|'
...
def p_statement(s, ctx, first_statement=0):
    ...
    if s.sy == 'ctypedef':
        ...
        if ctx.level not in (
        'module', 'class', 'c_class', 'function', 'property', 'module_pxd', 'c_class_pxd', 'other'):
...
def _reject_cdef_modifier_in_py(s, name):
    ...
    if s.sy == 'IDENT' and name in _CDEF_MODIFIERS:
        ...
        s.error("Cannot use cdef modifier '%s' in Python function signature. Use a decorator instead." % name,
                fatal=False)
